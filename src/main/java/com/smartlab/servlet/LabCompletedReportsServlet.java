...
@WebServlet("/LabCompletedReportsServlet")
public class LabCompletedReportsServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException {
        ...
        try {
            req.setAttribute("appointments", appointmentDAO.getLabCompletedAppointments(labUserId));
        } catch (SQLException ex) {
            getServletContext().log("Load completed reports failed", ex);
            req.setAttribute("appointments", Collections.emptyList());
            req.setAttribute("error", "Unable to load completed reports.");
        }
        req.getRequestDispatcher("/lab/completed_reports.jsp").forward(req, resp);
    }
